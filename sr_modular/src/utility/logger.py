...
class Logger:
    def __init__(self, log_dir="./logs", log_file_prefix="gp_run"):
        ...
        # Header per la tabella dei messaggi (Algorithm strategies track)
        self.messages_fields = [
            "timestamp",
            "message"
        ]
        # Liste interne per accumulare i log
        self.metrics_logs = []
        self.messages_logs = []
    ...
    def log_metrics(self, generation=None, best_fitness=None, avg_fitness=None, diversity=None, complexity=None,
        ...
        """Accumulare una riga di metriche nella lista interna."""
        row = {
    ...
    def log_message(self, message):
        """
        Accumula una riga di messaggio nella lista interna.
        Args:
            message (str or list): Messaggio singolo o lista di messaggi (che verranno concatenati con " | ").
        """
    ...
    def info(self, message, generation=None, best_fitness=None, avg_fitness=None, diversity=None, complexity=None,
             strategies=None, local_search=None):
        """
        Log informational messages to console e accumula le righe nelle liste interne.
        Args:
            message (str or list): Un messaggio singolo o una lista di messaggi.
        """
        if isinstance(message, list):
            message = " | ".join(message)
    ...
    def generate_summary(self, stats, best_expression, total_time, start_time, end_time, reason, success=True):
        ...
        # Log summary sulla console e memorizza il riepilogo nei messaggi
    ...
    def flush_logs(self):
        """
        Scrive in un unico file CSV le due tabelle: Metriche e Messaggi,
        separandole con righe di intestazione.
        """
        with open(self.log_file, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
            # Scrive la tabella delle metriche
            ...
            writer.writerow([])  # Riga vuota di separazione
            # Scrive la tabella dei messaggi
